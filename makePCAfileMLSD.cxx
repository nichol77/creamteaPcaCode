...
#define density_poly 1.060*pow(10.,-3);        // density of polystyrene g/mm^3
...
Double_t Evaluate_delta (double x_0, double z_0, double m, double q, double m2, double sign){
	Double_t z[3], x[3];
	Double_t delta;
	double TriHeightmm = scintTriHeight*1000.;
	double TriBasemm = scintTriBase*1000.;
	z[0] = (z_0 - (sign*TriHeightmm/2.) + (2*TriHeightmm*(q - x_0)/TriBasemm) )/(1. - (2*TriHeightmm*m/TriBasemm)); 
	x[0] = m*z[0] + q;
	z[1] = (z_0 - (sign*TriHeightmm/2.) - (2*TriHeightmm*(q - x_0)/TriBasemm) )/(1. + (2*TriHeightmm*m/TriBasemm)); 
	x[1] = m*z[1] + q;
	z[2] = z_0 + (sign*TriHeightmm/2.);
	x[2] = m*z[2] + q;
	int j1 = 0;
	for (int j = 0; j < 3; j++){
	 if (x[j] >= (x_0 - (TriBasemm/2. + 0.1)) && x[j] <= (x_0 + (TriBasemm/2.+ 0.1))){
	  if (z[j] >= (z_0 - (TriHeightmm/2.+ 0.1)) && z[j] <= (z_0 + (TriHeightmm/2.+ 0.1))){
	   x[j1] = x[j];
	   z[j1] = z[j];
	   j1++;
	   }
	...
	if (j1<2) return 0.;
	delta = (TMath::Sqrt(TMath::Power(x[0] - x[1],2.) + TMath::Power(z[0] - z[1],2.))*TMath::Sqrt(1.+m2*m2))*density_poly;
	return delta;
}


void meanAndRMS (int nPoints, double x[], double meanRMS [2]){
	meanRMS[0] = 0.; //mean
	meanRMS[1] = 0.; //RMS
	...
	meanRMS[0]/=nPoints;
	meanRMS[1] = TMath::Sqrt((meanRMS[1]/nPoints) - (meanRMS[0]*meanRMS[0]));
...
int main(int argc, char**argv)
{
   ...
   scintChain->SetBranchAddress("intMom",intMom);
   ...
   Double_t meanEnergyDep=0;
   Double_t RMSEnergyDep=0;
   Double_t EvalEng=0;
   Double_t dxTrue = 0.;
   Double_t dyTrue = 0.;
   Double_t dxReco = 0.;
   ...
   pcaTree->Branch("EvalEng",&EvalEng,"EvalEng/D");
   pcaTree->Branch("meanEnergyDep", &meanEnergyDep,"meanEnergyDep/D");
   pcaTree->Branch("RMSEnergyDep", &RMSEnergyDep,"RMSEnergyDep/D");
   pcaTree->Branch("dxTrue", &dxTrue,"dxTrue/D");
   pcaTree->Branch("dyTrue", &dyTrue,"dyTrue/D");
   pcaTree->Branch("dxReco", &dxReco,"dxReco/D");
   pcaTree->Branch("dyReco", &dyReco,"dyReco/D");
   ...
   Double_t AmeanEnergyDep = 0.;
   Double_t ARMSEnergyDep = 0.;
   ...
   Absorbed->Branch("EvalEng",&EvalEng,"EvalEng/D");
   Absorbed->Branch("meanEnergyDep",&AmeanEnergyDep,"meanEnergyDep/D");
   Absorbed->Branch("RMSEnergyDep",&ARMSEnergyDep,"RMSEnergyDep/D");
...
Int_t status=scintChain->GetEntry(entry);
    ...
    Double_t zxSignTop[MAX_SCINT_HITS];
    Double_t zySignTop[MAX_SCINT_HITS];
    Double_t zxSignBot[MAX_SCINT_HITS];
    Double_t zySignBot[MAX_SCINT_HITS];
    Double_t energyDepReco[MAX_SCINT_HITS] = {0.};
    Double_t meanRMS[2];
...
//       }
	...
	zxSignTop[countXTop-1]=TMath::Power(-1., st);	  
	lastStrip=st;
	//	std::cout << pl << "\t" << st << "\t"  <<  x 
	//		<< "\t" << z << "\t" << w << "\n";
      }
	...
	zySignTop[countYTop-1]=TMath::Power(-1., st);	  
	lastStrip=st;
	//	std::cout << pl << "\t" << st << "\t"  <<  y 
	//		<< "\t" << z << "\t" << w << "\n";
      }
   ...
   for(int testPlane=planesPerSide;testPlane<2*planesPerSide;testPlane+=2) {
	...
	zxSignBot[countXBot-1]=TMath::Power(-1., st);	  
	lastStrip=st;
	//	std::cout << pl << "\t" << st << "\t"  <<  x 
	//		<< "\t" << z << "\t" << w << "\n";
      }
	...
	zySignBot[countYBot-1]=TMath::Power(-1., st);	  
	lastStrip=st;
	//	std::cout << pl << "\t" << st << "\t"  <<  y 
	//		<< "\t" << z << "\t" << w << "\n";
      }
    ...
    if(topPlanes>=4) {
		  ...
		  recoFitValY=iterativelyFitXZWithWeights(countYTop,yTopAvgY,yTopAvgZ,yTopAvgW,recoGradYZ);
	       ...
	       yzCutReco[0]=recoGradYZ[1];
      		...
      		Int_t countReco = 0;
		Double_t delta = 0.;
//		for (int i = 0; i<countXTop; i++){
//		delta = Evaluate_delta (xTopAvgX[i], xTopAvgZ[i], xzGradReco[0], xzCutReco[0], yzGradReco[0], zxSignTop[i]);
//		if (delta!=0.){
//		 energyDepReco[countReco] = 1./delta*xTopAvgW[i];
//		countReco++;
//			}
//		}

//		for (int i = 0; i<countYTop; i++){
//		delta = Evaluate_delta (yTopAvgY[i], yTopAvgZ[i], yzGradReco[0], yzCutReco[0], xzGradReco[0], zySignTop[i]);
//		if (delta!=0.){
//		 energyDepReco[countReco] = 1./delta*yTopAvgW[i];
//		countReco++;
//			}
//		}
...

		  {
		     ...
		     float modp = sqrt(1.0+a1*a1);  // 1/cos(theta0)
		     float modq = sqrt(1.0+a2*a2);  // 1/cos(theta1)
	    
		     thetaxzTrue = atan((a1 - a2)/(1.0 + a1*a2));

			double Lxy = sqrt(1. + (xzGradTrue[0]*xzGradTrue[0]) + (yzGradTrue[0]*yzGradTrue[0]));
	    		double xp = xzGradTrue[0]*(-verticalSeparation*1000./2.) + xzCutTrue[0];
	    		double x1 = xzGradTrue[1]*(-verticalSeparation*1000./2.) + xzCutTrue[1];
			dxTrue = (xp - x1)*Lxy/(cos(thetaxzTrue)*modp*modq);
		     ...
		     thetayzTrue = atan((a1 - a2)/(1.0 + a1*a2));
			xp = yzGradTrue[0]*(-verticalSeparation*1000./2.) + yzCutTrue[0];
	    		x1 = yzGradTrue[1]*(-verticalSeparation*1000./2.) + yzCutTrue[1];
			dyTrue = (xp - x1)*Lxy/(cos(thetayzTrue)*modp*modq);
...
//		for (int i = 0; i<countXBot; i++){
//		delta = Evaluate_delta (xBotAvgX[i], xBotAvgZ[i], xzGradReco[0], xzCutReco[0], yzGradReco[0], zxSignBot[i]);
//		if (delta!=0.){
//		 energyDepReco[countReco] = 1./delta*xBotAvgW[i];
//		countReco++;
//			}
//		}

//		for (int i = 0; i<countYBot; i++){
//		delta = Evaluate_delta (yBotAvgY[i], yBotAvgZ[i], yzGradReco[0], yzCutReco[0], xzGradReco[0], zySignBot[i]);
//		if (delta!=0.){
//		 energyDepReco[countReco] = 1./delta*yBotAvgW[i];
//		countReco++;
//			}
//		}

		  {
		     ...
		     thetaxzReco = atan((a1 - a2)/(1.0 + a1*a2));	    

			double Lxy = sqrt(1. + (xzGradReco[0]*xzGradReco[0]) + (yzGradReco[0]*yzGradReco[0]));
	    		double xp = xzGradReco[0]*(-verticalSeparation*1000./2.) + xzCutReco[0];
	    		double x1 = xzGradReco[1]*(-verticalSeparation*1000./2.) + xzCutReco[1];
			dxReco = (xp - x1)*Lxy/(cos(thetaxzReco)*modp*modq);
		     ...
		     thetayzReco = atan((a1 - a2)/(1.0 + a1*a2));	    

	    		xp = yzGradReco[0]*(-verticalSeparation*1000./2.) + yzCutReco[0];
	    		x1 = yzGradReco[1]*(-verticalSeparation*1000./2.) + yzCutReco[1];
			dyReco = (xp - x1)*Lxy/(cos(thetayzReco)*modp*modq);
		  }
	    ...
	    } // end of if (topPlanes >= 4)

	 } // end of if (numScintHits>3)
